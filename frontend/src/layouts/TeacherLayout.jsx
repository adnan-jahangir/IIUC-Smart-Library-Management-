...
const TeacherLayout = () => {
  ...
  const navLinks = [
    { name: 'Dashboard', path: '/teacher/dashboard', icon: <Library className="w-5 h-5" /> },
    { name: 'Search Books', path: '/teacher/search', icon: <Search className="w-5 h-5" /> },
    { name: 'My Borrowed', path: '/teacher/my-books', icon: <BookOpen className="w-5 h-5" /> },
  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 px-4 space-y-1">
        <p className="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Faculty Portal</p>
        {navLinks.map((link) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${
                isActive ? 'bg-indigo-50 text-indigo-700 shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-indigo-600'
              }`}
            >
      ...
      <div className="p-4 border-t border-slate-200 mt-auto">
        <Link to="/teacher/profile" className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-slate-50 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex">
      ...
      <div
        ...
        aria-hidden="true"
      />
      <aside className={`w-72 bg-white border-r border-slate-200 flex flex-col fixed h-full z-50 lg:hidden transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center justify-between border-b border-slate-200">
          ...
          <button className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg" onClick={() => setIsMobileMenuOpen(false)} aria-label="Close menu">
            <X className="w-6 h-6" />
          </button>
        </div>
        {sidebarContent}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3 border-l border-slate-200 pl-4">
                  ...
                  <div className="hidden sm:block text-sm">
                    <p className="font-semibold text-slate-800">{user?.name || 'Teacher'}</p>
                    <p className="text-slate-500 text-xs">ID: {user?.customId || 'T1000'}</p>
                  </div>
                </div>
              </div>
            </div>
